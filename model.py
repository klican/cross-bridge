...
def initConsts():
    ...
    # To je meniaca sa hladina fosfatu
    # voi nahrada 
    states[11] = 0
    # toto je nove Pi 
    #states[12] = 800
...
def computeRates(voi, states, constants):
    ...
    if states[11] > 2000:
        ...
        print(t)
        rates[12] = 0.002    
        Atp = 6.549e-4*t**3 - 0.02305*t**2 - 0.104837*t + 6.9485
        PCr = -0.01259 + 12.339*(math.exp(-0.92559*t)) + 0.96819*(math.exp(-0.078496*t))
        Cr = 22.2 - PCr
        ph = 6.12457 - 0.56697*(math.exp(-0.19015*t)) + 1.5377*(math.exp(-0.18462*t))
        H = 1e3*10**(-ph)
        ck = 1.66e6
        ak = 1.05
        Adp = (Atp*Cr)/(PCr*H*ck)
        Amp = (Adp**2*ak)/Atp
        Pi_tot = 0.8 + 3*7.08 + 2*0.036 + 13.3 + 1.84e-4
        Pi = Pi_tot - (3*Atp + 2*Adp + Amp + PCr)
        constants[60] = 1.00000e+07*(power(10.0000, -ph))
        constants[63] = ((power(constants[17], constants[16])+power(constants[62], constants[16]))/(power(constants[17], constants[16])+power(constants[60], constants[16])))*(constants[7]*(power(constants[3], (constants[18]-37.0000)/10.0000)))
        constants[42] = Pi*1000
        constants[43] = Atp*1000
        #rates[12] = 5.84117
        #rates[13] = -0.0003
    #print(states[12])
    #print(states[13])
    ...
    # namiesto constansts[42] - xPi je states[11], ktore sa kazdym cyklom
    # spocitava
    #if states[11] < 2000:
    algebraic[29] = constants[42]*(algebraic[16]/constants[39])
    #else:
    #    algebraic[29] = Pi*(algebraic[16]/constants[39])
    ...
    #else:
    #    algebraic[30] =  PH*(algebraic[24]/constants[62])*((constants[38]+constants[41])/constants[41])*(constants[44]/(constants[38]+constants[44]))
    ...
    #else:
    #    algebraic[28] = Atp*(algebraic[26]/constants[40])*((constants[38]+constants[41])/(constants[38]+Adp))
